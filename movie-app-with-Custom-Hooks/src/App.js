...
import useMoviesCustom from './Components/useMoviesCustom/useMoviesCustom';
import LocalStorageCustom from './Components/LocalStorageCustom/LocalStorageCustom';
...
function App() {
  ...
  const [watchMovie, setWatchMovie] = LocalStorageCustom([], "watchMovie");
  ...
  const runtime = average(watchMovie.map((item) => item.runtime));
  ...
  const { movies, loading, error } = useMoviesCustom(query);
      ...
      <div className='items'>
        ...
        <div className='watch'>
          {selectedID ?
            ...
            :
            <WatchMovie watchMovie={watchMovie} avgImdbrating={avgImdbrating} userRating={userRating} runtime={runtime} deleteHandler={deleteHandler} />}
        </div>
